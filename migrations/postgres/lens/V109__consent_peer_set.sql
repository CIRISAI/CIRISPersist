...
-- Postgres dialect. SQLite parity: sqlite/lens/V109.
--
...
-- see `pg_project_consent_peer_set`):
...
--
-- Backend ASYMMETRY (documented): `source_attestation_id` is TEXT (not
-- `UUID` FK'd to `cirislens.federation_attestations`) on BOTH backends —
-- deliberately no FK, so a revocation-fold DELETE that names an id this
-- node has not (yet) seen the grant for is a plain no-op, never an FK
-- error (replication order is not guaranteed).
CREATE TABLE cirislens.consent_peer_set (
