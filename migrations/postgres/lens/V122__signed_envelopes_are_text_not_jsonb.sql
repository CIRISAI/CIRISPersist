-- V122 — v30.13.0 (CIRISPersist#644): **the signature-covered envelope columns
...
COMMENT ON COLUMN cirislens.federation_keys.registration_envelope IS
    'v30.13.0 (CIRISPersist#644) — TEXT, not JSONB: the exact bytes the '
...
COMMENT ON COLUMN cirislens.federation_attestations.attestation_envelope IS
    'v30.13.0 (CIRISPersist#644) — TEXT, not JSONB. Query sites that need to '
...
COMMENT ON COLUMN cirislens.federation_revocations.revocation_envelope IS
    'v30.13.0 (CIRISPersist#644) — TEXT, not JSONB. V118 reads `revoked_after` '
