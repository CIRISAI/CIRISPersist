...
-- (v32.1.0, CIRISPersist#606, SQLite).
--
-- Mirrors migrations/postgres/lens/V128__trace_events_admission_instant.sql.
--
...
--
-- NULLABLE, NO BACKFILL, deliberately. A node that upgrades has genuinely never
...
ALTER TABLE trace_events ADD COLUMN admitted_at TEXT;

CREATE INDEX IF NOT EXISTS trace_events_admitted_at
    ON trace_events (admitted_at DESC)
