...
--
-- POSTGRES PARITY: migrations/postgres/lens/V150__cohort_target_generated_column.sql
--
-- # Why
--
...
--
-- VIRTUAL, not STORED: sqlite computes it per read and the index below
-- materializes what the gate needs. A STORED generated column would rewrite
-- the table.
...
ALTER TABLE federation_attestations
    ADD COLUMN cohort_target TEXT
        GENERATED ALWAYS AS (
            COALESCE(
                json_extract(attestation_envelope, '$.community_id'),
                json_extract(attestation_envelope, '$.community_key_id'),
                json_extract(attestation_envelope, '$.cohort_key_id'),
                json_extract(attestation_envelope, '$.family_key_id')
            )
        ) VIRTUAL;

-- A FUTURE REBUILD MUST CARRY THIS. If a later migration transcribes
-- federation_attestations into a `_new` table and renames (the V141 shape,
...
CREATE INDEX IF NOT EXISTS federation_attestations_cohort_target
    ON federation_attestations (cohort_scope, cohort_target)
