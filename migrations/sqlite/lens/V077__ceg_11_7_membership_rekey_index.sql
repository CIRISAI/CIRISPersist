...
--        (CIRISPersist#161 Ask 2/4, v6.1.0) — SQLite dialect. Postgres
--        parity: postgres/lens/V077. See that file for the full rationale.
--
...
--   WHERE cohort_scope = ? AND recipient_key_id IN (…);
--
...
CREATE INDEX federation_blob_key_grants_by_scope_recipient
    ON federation_blob_key_grants (cohort_scope, recipient_key_id);
