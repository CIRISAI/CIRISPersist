//! JSON path resolution — dot-notation paths to `serde_json::Value`.
//!
//! Lifted verbatim from CIRISLensCore `src/extract/json_path.rs`
//! (v0.6.0-α3). Used by [`super::static_extract`] to walk component
//! payloads + by future v0.6.x dynamic-field extraction rules.
...
/// ```
/// # use serde_json::json;
/// # use ciris_persist::pipeline::extract::resolve_json_path;
...
/// Convert a JSON value to a string representation for database
/// storage.
...
mod tests {
    ...
    #[test]
    fn test_nested_path() {
        let data = json!({"csdma": {"plausibility_score": 0.95, "confidence": 0.8}});
        assert_eq!(
    ...
    fn test_deep_nested_path() {
        let data = json!({
            "system_snapshot": {"agent_profile": {"name": "TestAgent"}}
        });
        assert_eq!(
