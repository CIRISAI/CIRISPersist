// safetensors mmap path requires `unsafe` for the OS-level mapping —
// the file may be mutated externally and Rust can't prove memory safety.
// Same shape as candle-transformers' own examples. Scoped to this file.
#![allow(unsafe_code)]
#![allow(missing_docs)]

//! DistilBERT-multilingual loader for token classification.
//!
//! Pairs with [`super::xlm_r_loader`]. Same input/output
...
impl DistilBertTokenClassifier {
    ...
    fn from_files(
        ...
        let backbone =
            DistilBertModel::load(vb.clone(), &candle_config).context("DistilBertModel::load")?;
