// safetensors mmap path requires `unsafe` — same rationale as
// distilbert_loader.rs. Scoped to this file.
#![allow(unsafe_code)]
#![allow(missing_docs)]
...
impl XLMRTokenClassifier {
    ...
    fn from_files(
        ...
        let candle_config: Config =
            serde_json::from_str(&config_str).context("parse candle Config from config.json")?;
...
fn parse_labels(cfg: &HfConfig) -> Result<Vec<String>> {
    ...
    if cfg.num_labels == Some(7) || cfg.num_labels.is_none() {
        return Ok(["O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC"]
            .iter()
            .map(|s| s.to_string())
            .collect());
    }
