...
/// v0.6.0-α2/α4 stub: when the `scrub-ner` feature is off, the NER
/// backend is never configured and `full_traces` traces are
/// rejected with `ScrubError::NerNotConfigured`.
...
/// new ruleset outputs.
#[allow(unreachable_code, unused_variables, clippy::needless_late_init)]
...
// v0.6.0-α4 — cache types compile unconditionally (cheap; parking_lot
// is small + well-maintained). When `scrub-ner` is off, scrub_batch's
// is_configured() guard returns early and the cache stays unallocated.
...
#[cfg_attr(not(any(feature = "scrub-ner", test)), allow(dead_code))]
...
mod backend {
    ...
    use super::super::distilbert_loader::{DistilBertSource, DistilBertTokenClassifier};
    use super::super::xlm_r_loader::{ModelSource as XlmrSource, XLMRTokenClassifier};
    ...
    fn run_batch(
        ...
        for (i, &(origin_idx, byte_offset_in_origin, _)) in batch.iter().enumerate() {
            ...
            for (s, e, tag) in chunk_spans {
                per_text_spans[origin_idx].push((
                    s + byte_offset_in_origin,
                    e + byte_offset_in_origin,
                    tag,
                ));
            }
        }
...
#[cfg(feature = "scrub-ort")]
mod ort_backend {
    use super::super::ort_loader::{OrtSource, OrtTokenClassifier};
    use super::{collapse_bio, replace_spans, ScrubError, ScrubStats};
    use ndarray::Array2;
    use parking_lot::Mutex;
    use std::sync::OnceLock;
    ...
    fn run_batch(
        ...
        let (logits_flat, batch_out, seq_out, num_labels) = backend
            .model
            .forward(ids, mask)
            .map_err(|e| ScrubError::NerFailed(format!("ort forward: {e:#}")))?;
        ...
        for (b, row) in label_ids_2d.iter_mut().enumerate().take(batch_out) {
            for s in 0..seq_out {
