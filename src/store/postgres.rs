...
impl crate::federation::FederationDirectory for PostgresBackend {
    ...
    async fn add_family_member(
        ...
        // Idempotent roster-grow. The read-modify-write races a concurrent
        // add only if two members are admitted at the same instant; the
        // `WHERE persist_row_hash = $4` guard makes the UPDATE a no-op on a
        // stale read (0 rows affected → retry signalled), so we never lose
        // a member to a lost update.
        ...
        let prior_hash = family.persist_row_hash.clone();
        family.members.push(member);
        family.persist_row_hash = crate::federation::types::compute_persist_row_hash(&family)?;
        let members_value = serde_json::to_value(&family.members)
            .map_err(|e| crate::federation::Error::Backend(format!("members serialize: {e}")))?;
        let client = self
            .get_client()
            .await
        ...
        if affected == 0 {
            return Err(crate::federation::Error::Backend(format!(
                "add_family_member on {family_key_id:?} lost a concurrent update (roster changed under the read); retry"
            )));
        }
        Ok(true)
    }
...
impl crate::federation::BlobStorage for PostgresBackend {
    ...
    async fn list_at_rest_blobs_for_recipients(
        ...
        rows.iter()
            .map(|r| {
                let v: Vec<u8> =
                    r.safe_get_with("at_rest_sha256", crate::federation::BlobError::Backend)?;
...
impl crate::read::ReadEngine for PostgresBackend {
    ...
    async fn aggregate_scoring_factors_batch(
        ...
        if let Some(cached) = cache.try_get(&key) {
            // Stored entry carries `cache_hit: false`; flip it on each
            // served clone (FSD §6.1 / §7.4). `evaluated_at_unix_ms` is
            // preserved — the cached evaluation time, not now().
            //
            // The entry is shared set-wise (sorted-agent key); remap the
            // cached set onto the caller's *input* order to honour the
            // batch contract (one aggregate per agent in input order).
            let mut hit = (*cached).clone();
            for a in &mut hit {
                a.cache_hit = true;
            }
...
impl PostgresBackend {
    ...
    async fn aggregate_scoring_factors_uncached(
        ...
        Ok(crate::read::ScoringFactorAggregate {
            ...
            // CIRISPersist#195 — compute-time honesty fields.
    ...
    async fn scoring_ingest_watermark_ms(
        ...
        let client = self
            ...
            .await
            .map_err(|e| crate::read::Error::Backend(format!("pool: {e}")))?;
        // agent_id_hash = ANY($1); scope predicate rebases to $2+.
        let (scope_frag, sparams) = crate::store::scope_bind::scope_predicate_pg(
            scope,
            "cohort_scope",
            "cohort_target_id",
        ...
        let sql = format!(
            "SELECT MAX(ts) AS w FROM cirislens.trace_events \
             WHERE agent_id_hash = ANY($1) AND {scope_frag}"
        );
        let mut params: Vec<Box<dyn tokio_postgres::types::ToSql + Sync + Send>> =
            vec![Box::new(agent_id_hashes.to_vec())];
        params.extend(sparams);
        let params_ref: Vec<&(dyn tokio_postgres::types::ToSql + Sync)> =
            params.iter().map(|p| p.as_ref() as _).collect();
        let row = client
            .query_one(&sql, &params_ref[..])
            .await
            .map_err(|e| crate::read::Error::Backend(format!("scoring_ingest_watermark: {e}")))?;
        // MAX over an empty set is NULL → 0.
        let w: Option<chrono::DateTime<chrono::Utc>> = row
            .try_get::<_, Option<chrono::DateTime<chrono::Utc>>>("w")
            .map_err(|e| crate::read::Error::Backend(format!("watermark decode: {e}")))?;
        Ok(w.map(|t| t.timestamp_millis()).unwrap_or(0))
    }
}
...
mod tests {
    ...
    /// membership-change re-key keystone on Postgres (parity with the
    /// sqlite `rekey_*` tests): a newcomer family member gains access to a
    /// pre-existing family blob via the retroactive ADD walk; a keyless
    /// newcomer is fail-secure excluded; both are idempotent; the
    /// membership `hard_case:*` rows are emitted.
    #[tokio::test]
    #[serial_test::serial(postgres)]
    async fn rekey_family_member_add_keystone_pg() {
        use crate::federation::at_rest_cascade::orchestrate::{
        ...
        let carol = format!("carol-{s}");
        let bob = format!("bob-{s}");
        let alice_p = format!("alice-phone-{s}");
        let carol_p = format!("carol-phone-{s}");
        let bob_p = format!("bob-phone-{s}");
        for kid in [&fam, &alice, &carol, &bob, &alice_p, &carol_p, &bob_p] {
            backend
                .put_public_key(crate::federation::SignedKeyRecord {
                    record: fix_section_i_key(kid, "acme", now, true),
                })
        ...
        // write time, so the blob reaches only alice-phone; bob's keyed
        // occurrence and carol's keyless one are registered afterwards.
        backend
            .put_family(family(vec![&alice, &bob, &carol]))
            .await
            .unwrap();
        ...
        let rk = rekey_family_member_add(&backend, &fam, &bob, observed)
            .await
            .unwrap();
        assert_eq!(rk.blobs_scanned, 1);
        assert_eq!(rk.granted, vec![(bob_p.clone(), 1)]);
        assert!(rk.excluded.is_empty());
        let recovered = read_for_viewer(&backend, &result.at_rest_sha256, &bob_p)
            .await
            .unwrap();
        assert_eq!(recovered, plaintext);

        // Idempotent: a second walk at the same instant adds zero grants.
        let rk2 = rekey_family_member_add(&backend, &fam, &bob, observed)
            .await
            .unwrap();
        assert_eq!(rk2.granted, vec![(bob_p.clone(), 0)]);
        ...
        let rk3 = rekey_family_member_add(&backend, &fam, &carol, observed2)
            .await
            .unwrap();
        assert!(rk3.granted.is_empty());
        assert_eq!(rk3.excluded, vec![carol_p.clone()]);
        assert!(read_for_viewer(&backend, &result.at_rest_sha256, &carol_p)
            .await
            .is_err());

        // hard_case rows present for this family (filter by target via the
        // membership-change kind, then narrow to our family_key_id).
        let mc = backend
        ...
        assert!(mc
            .iter()
            .any(|e| e.target_key_id.as_deref() == Some(fam.as_str())
                && e.subject_key_id.as_deref() == Some(carol_p.as_str())));
        let exc = backend
            .list_hard_case_events(HardCaseFilter {
    ...
    /// a genuinely-new member (NOT on the roster at write time) is added by
    /// `rekey_family_member_add` (exercising the `add_family_member` UPDATE
    /// path + concurrent-guard) and is then served a FUTURE family write.
    #[tokio::test]
    #[serial_test::serial(postgres)]
    async fn add_family_member_grows_roster_and_serves_future_writes_pg() {
        use crate::federation::at_rest_cascade::orchestrate::{
            encrypt_and_cascade, read_for_viewer, rekey_family_member_add,
        };
        use crate::federation::types::cohort_scope::FAMILY;
        ...
        let alice_p = format!("alice-phone-{s}");
        let bob_p = format!("bob-phone-{s}");
        for kid in [&fam, &alice, &bob, &alice_p, &bob_p] {
            backend
                .put_public_key(crate::federation::SignedKeyRecord {
                    record: fix_section_i_key(kid, "acme", now, true),
                })
        ...
        let rk = rekey_family_member_add(&backend, &fam, &bob, chrono::Utc::now())
            ...
            .unwrap();
        assert_eq!(rk.granted, vec![(bob_p.clone(), 1)]);
        assert!(read_for_viewer(&backend, &blob1.at_rest_sha256, &bob_p)
            .await
            .is_ok());
    ...
    /// `cache_hit: true`, preserved `evaluated_at_unix_ms`. The singular
    /// streaming path (batch-of-one) shares the entry.
    #[tokio::test]
    #[serial_test::serial(postgres)]
    async fn read_section_e_scoring_factors_cache_hit() {
        let Some(dsn) = pg_dsn() else {
            eprintln!("skipping: CIRIS_PERSIST_TEST_PG_URL unset");
            return;
        };
        ...
        use crate::read::ReadEngine;
        let suffix = uuid_like();
        let aid = format!("agent-§e-cachehit-{suffix}");
        let now = chrono::Utc::now();
        insert_section_a_fixture_trace(
            &backend,
            &format!("trace-§e-cachehit-{suffix}"),
            &aid,
            Some(&aid),
            Some("d"),
            now,
        ...
        let before_hits = backend.scoring_factors_cache().stats().hits;
        let a = backend
            .aggregate_scoring_factors_batch(
        ...
        // Singular path shares the entry.
        let c = backend
            .aggregate_scoring_factors(
                &aid,
                window,
        ...
        assert_eq!(
            backend.scoring_factors_cache().stats().hits,
            before_hits + 2
        );
    }
    ...
    #[tokio::test]
    #[serial_test::serial(postgres)]
    async fn read_section_e_scoring_factors_cache_watermark_invalidation() {
        let Some(dsn) = pg_dsn() else {
            eprintln!("skipping: CIRIS_PERSIST_TEST_PG_URL unset");
            return;
        };
        ...
        use crate::read::ReadEngine;
        let suffix = uuid_like();
        let aid = format!("agent-§e-watermark-{suffix}");
        let now = chrono::Utc::now();
        insert_section_a_fixture_trace(
            &backend,
            &format!("trace-§e-watermark-{suffix}-0"),
            &aid,
            Some(&aid),
            Some("d"),
            now,
        ...
        // the max(ts) watermark → new key → miss → recompute).
        insert_section_a_fixture_trace(
            &backend,
            &format!("trace-§e-watermark-{suffix}-1"),
            &aid,
            Some(&aid),
            Some("d"),
        ...
        assert!(after[0].evaluated_at_unix_ms >= a[0].evaluated_at_unix_ms);
    }

    /// Agent-set ORDER INDEPENDENCE: the same set in a different caller
    /// order shares the entry (hit) and the result is remapped to the
    /// caller's input order.
    #[tokio::test]
    #[serial_test::serial(postgres)]
    async fn read_section_e_scoring_factors_cache_order_independent() {
        let Some(dsn) = pg_dsn() else {
            eprintln!("skipping: CIRIS_PERSIST_TEST_PG_URL unset");
            return;
        };
        ...
        use crate::read::ReadEngine;
        let suffix = uuid_like();
        let aid_a = format!("agent-§e-order-A-{suffix}");
        let aid_b = format!("agent-§e-order-B-{suffix}");
        let now = chrono::Utc::now();
        for aid in [&aid_a, &aid_b] {
            insert_section_a_fixture_trace(
                &backend,
                &format!("trace-§e-order-{aid}"),
                aid,
                Some(aid),
                Some("d"),
                now,
        ...
        let before_misses = backend.scoring_factors_cache().stats().misses;
        let first = backend
            .aggregate_scoring_factors_batch(
                &[aid_a.clone(), aid_b.clone()],
                window,
        ...
        // Reversed caller order → same key → hit; result remapped to the
        // caller's (reversed) input order.
        let second = backend
            .aggregate_scoring_factors_batch(
                &[aid_b.clone(), aid_a.clone()],
                window,
                None,
                crate::scope::CallerScope::Unauthenticated,
            )
        ...
        assert_eq!(second[0].agent_id_hash, aid_b, "result follows input order");
        assert_eq!(second[1].agent_id_hash, aid_a);
        assert_eq!(
            backend.scoring_factors_cache().stats().misses,
            before_misses + 1,
            "exactly one compute for the set"
        );
    }
    ...
    // Gated behind CIRIS_PERSIST_TEST_PG_URL; skipped when unset.
    #[tokio::test]
    #[serial_test::serial(postgres)]
