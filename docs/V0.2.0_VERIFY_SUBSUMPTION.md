...
Realistic schedule from a single-developer perspective:

| Day | Work |
|---|---|
| 1-2 | `ciris-verify-core` pin bump v1.8.0 → v1.8.4. Verify the existing test suite passes against the new pin. |
| 3-5 | Add proxy methods to `Engine`. Smoke tests for each. |
| 6-7 | Update `lens` (CIRISLens) integration tests against the persist API; confirm direct `ciris-verify` imports can be dropped from lens code path without behavior change. |
| 8-9 | CHANGELOG entry; release notes; v0.2.0 cut. |
| 10+ | Lens / agent / bridge consumers migrate at their own pace; persist v0.2.x patches absorb any consumer-found bugs. |
...
the prerequisite that makes v0.3.0 ergonomic.
