...
| AV-82 | AV-79's gate answered ONE question while reading as a verdict on the set: it matched `capacity:*` and nothing else, so every verify-owned trust signal ABOUT a subject — `attestation:registry_consensus`, `attestation:license_validity`, `cert_validity:*`, `rollback_detected:*` — landed with no consent asked, feeding the same reputation surface. A subject could decline `analyze`, believe they had opted out of being scored, and still accumulate third-party trust signals | v25.1.0 — SHIPPED: the gate now classifies through `consent_gated_family`, which asks verify's AUTHORITATIVE registry (`ciris_verify_core::federation_provenance::dim::lookup`, v11.0.0) and gates every family verify itself classifies `ConsentClass::ConsensualReputation`. Typed refusal `Error::ConsentGateRefused` names WHICH rule and WHICH dimension. Both wire shapes, all three backends | The set is **derived, never transcribed**: persist holds no list, so a family added upstream is gated (or exempted) under verify's own classification instead of slipping past a hand-copied list — the two-lists-that-disagree class (#541/#532/#574). A pin test catches RECLASSIFICATION (a family changing consent class silently changes what this node refuses) so it is adjudicated, not inherited from a dependency bump. Verify's `SelfAttestation` class stays ungated: no third-party subject exists, so gating it would be the same category error as gating `detection:*` | ✅ **Shipped** — B7 `{gate} × {backend}` matrix, red-first (the unconsented claim was witnessed ADMITTED, including on the node's own `emit_attestation` surface) | CIRISPersist#569 |
