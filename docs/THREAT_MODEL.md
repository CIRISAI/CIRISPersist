...
| AV-27 | Identity churn via ephemeral keyring storage | Boot-time warn-on-ephemeral check: if `!is_hardware_available()` and predicted seed path matches container-writable-layer heuristic (`/home/`, `/root/`, `/tmp/`, `/var/cache/`, `/var/tmp/`), `tracing::warn!` with the path + fix prose. `Engine.keyring_path()` exposes predicted path for `/health` surfacing | Suppression via `CIRIS_PERSIST_KEYRING_PATH_OK=1` after operator audit; `INTEGRATION_LENS.md §11.5` deployment template guidance | **✓ Mitigated v0.1.7** | Upstream trait method ask: `HardwareSigner::storage_descriptor()` |
