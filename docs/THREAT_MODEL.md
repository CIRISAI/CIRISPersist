...
| AV-82 | **RAISED AND ADJUDICATED — not a vulnerability.** CIRISPersist#569 argued that AV-79's gate "answered one question while reading as a verdict on the set": it matched `capacity:*` and nothing else, leaving four verify-owned families (`attestation:registry_consensus`, `attestation:license_validity`, `cert_validity:*`, `rollback_detected:*`) admitting with no consent asked | v25.1.0 — the widening was WRITTEN, then HELD before merge. **CC 3.4.5 ratified the opposite** and dispositioned all four families individually: the first three are artifact-integrity verification ("a forger never consents to verification"); `rollback_detected:{revision_field}` is "an adversarial detector (−1-only polarity), on the abuse-response side of the line by construction". The rule: *consent-before-scoring binds the family that judges agents — `capacity:*` — never the families that verify artifacts.* The gate ships `capacity:*`-only | **Gating `rollback_detected:*` would have been a new hole, not a closed one**: it lets an adversary opt out of its own rollback detection by declining `analyze` — contradicting #569's own principle (never gate abuse-response), which it applied correctly to `detection:*` / `moderation:*` / `slashing:*` and missed on the one adversarial family inside verify's namespace. #569's method was right (derive from verify's registry, never a hand-copied list); its error was treating verify's `ConsentClass` as a ruling when verify itself calls it "a proposal from the measuring side, not a ruling". Nor is the narrowing a weakening: CC 3.4.5's reciprocity clause makes a declining subject unscoreable outright — `capacity:composite` undefined, every gate needing a capacity verdict fails closed | ✅ **Adjudicated** — B7 inverted into the witness that these families ADMIT unconsented on all three backends, plus `verify_dimension_registry_is_the_only_enumeration` as the standing adjudication record (red if verify reclassifies OR if a CC amendment moves a family) | CIRISPersist#569 / CC 3.4.5 |
| AV-83 | **Tier laundering through PROMOTE.** `Engine::attestation_promote` and the backends' `promote_attestation` / `promote_attestation_transformed` re-signed a row and flipped `tier` local→federation while running **no** put-gate at all — they validated `cohort_scope`, refused `(federation, self)`, and were idempotent, and that was the whole of it. Every tier-4 gate that no-ops at the local tier had therefore never been asked about a promoted row, so a row entered at `tier = "local"` and left federation-tier past AV-45, AV-77 and the moderation gates. The sharpest instance is a **CC 3.4.5 MUST violation**: the reciprocity clause says a subject that declines analysis "cannot be scored; its `capacity:composite` is undefined and MUST NOT be emitted", and this path minted exactly that row — and because `capacity:composite` is `min` over five factors (CC 3.1.8.1), one leaked row certifies five. Compounding it, the rule *"capacity is never local"* was already written, tested and shipped in `check_local_tier_eligibility` — but it runs only on `attestation_insert_local` / `attestation_upsert_local`, never on `put_attestation`, which accepts `tier = "local"` on every backend. Third occurrence of the SHIPPED-means-host-reachable class after AV-77 and #444's route table | v25.2.0 — SHIPPED, at **two** chokepoints because they close different things. (1) `check_capacity_never_local` — the rule lifted out of `check_local_tier_eligibility` into one predicate both doors ask, wired into `put_attestation` at AV-76 tier 1 on all three backends, on BOTH wire shapes; the refusal names the LOCAL-TIER rule, not consent, because the row is inadmissible even WITH a live grant. (2) `check_promotion_admission` — the tier-4 authority stack re-run against the row a promotion is about to store (`tier = federation` + the new placement), called from every backend's `promote_attestation` AND `promote_attestation_transformed` before any mutation, so `Engine::attestation_promote`, `promote_consented_backlog`, the pyo3 wrapper and the FFI capsule all inherit it. The rule for membership: *a promotion re-runs every tier-4 gate whose verdict is a function of state that can have changed since the local write, and that neither mutates the row nor assumes the row is not yet stored* — which admits the consent gate, no-moderator-federate, de-admission, delegated-duty, reserved-prefix, node-agency and user-target-steward gates, and excludes the §6.1 dedup (an early ACCEPT), `check_withdraws_admission` (it stamps a hash-covered field) and `check_single_node_owner_admission` (its doc pins "the incoming row is NOT yet stored"). `promote_attestation` now also CARRIES its `cohort_scope`, landing the placement in the same statement as the tier flip **v39.0.0 — the door moved and the gate moved with it.** `promote_attestation` is gone; `check_promotion_admission` is now called from `crossing::plan_enter_mesh`, which every backend's `enter_mesh` runs before any mutation, so the membership rule above holds unchanged at the one crossing there is. The placement no longer moves at that door at all (a tier crossing is byte-identical, CC 5.3.2.4.2), which retires the pre-stamp hazard by construction rather than by ordering. | **AV-45 is deliberately NOT in the promotion gate, and that is the residual.** An attestation carries a `cohort_scope` with no `cohort_target_id`, so AV-45's predicate refuses `family` / `community` whenever the target is `None` — always, on this table. Running it at promote would not CHECK a placement, it would make family/community placements unreachable and delete the #519/#510 audience plane: a product amputation wearing a security fix's clothes. The provenance also differs — AV-45 at `put_attestation` polices an inbound peer asserting an unverifiable cohort label, whereas a promotion republishes a row this node itself authored at an audience from its own signed grant. What remains true is that persist cannot PROVE a family/community placement on any path; closing that is a SCHEMA change (give the row a `cohort_target_id`), tracked separately rather than smuggled in here, and pinned by `promotion_does_not_prove_cohort_membership_589` so it cannot drift untrue in either direction. **AV-84 (CIRISPersist#592) took the answerable half of this residual**: the provenance sentence above — *"a promotion republishes a row this node itself authored"* — was the entire justification for AV-45's absence and was never enforced by anything; it is now a gate. What AV-84 does NOT close, and what keeps this residual open, is the schema half. Also note the placement-carrying change was NOT cosmetic: the old `set_attestation_cohort_scope`-then-promote two-step was safe only while promotion could not refuse, and the substrate state machine caught it as an I2a violation ("a REFUSED op must leave every existing row byte-identical") within one run of the gate landing | ✅ **Shipped** — B8 `{gate} × {backend}` matrix (capacity refused at the local door on both wire shapes; an ordinary local row still promotes; a DE-ADMITTED author's row refused at promote — the class, not the symptom; a refused promotion byte-identical; `(federation, self)` refused by the primitive; and a pre-v25.2.0 legacy local capacity row refused at promotion, then admitted once the subject grants `analyze` — the arm that would otherwise be unreachable from an empty corpus), plus `attestation_promote_refusal_does_not_prestamp_placement_589` (the ordering rule, made deterministic after mutation testing showed the state machine only rediscovers it by chance) and the substrate state machine's I2a/I5 across the trio | CIRISPersist#589 / CC 3.4.5 |
| AV-84 | **A promotion places a row into a cohort plane it has no standing in.** AV-83 re-ran the tier-4 stack at the promote door and left AV-45 out, because on `federation_attestations` AV-45 cannot ASK its question: the row carries a `cohort_scope` and no `cohort_target_id`, so the predicate refuses `family`/`community` on a `None` target — always. The put door answers that the only honest way it can (both placements refused outright), so promotion is the ONLY door those placements have ever had, and copying the put door's answer would delete the #519/#510 audience plane rather than check anything. #589 justified the asymmetry in prose — *"a promotion re-publishes a row this node itself authored … a self-declaration about its own content's visibility, not a claim about someone else's cohort"* — and **nothing enforced that sentence.** `attestation_promote` is a raw primitive that places ANY local-tier row, including one authored by and about a peer; `promote_consented_backlog` pages `list_local_tier_attestations` (`WHERE tier = 'local'`, no author predicate) so a peer's row is promoted under THIS node's grant and re-signed with THIS node's key; and `repair_stranded_scope_backlog` re-scopes an already-federation row to a grant's audience through `set_attestation_cohort_scope`, whose broaden-only guard skips only `self`/`family` and lets `community` straight through. So a node in no community could publish a stranger's verdict about a third party into the community plane under its own signature, past a stack that had just been audited | SHIPPED (CIRISPersist#592): `check_promotion_cohort_standing`, a PURE predicate (no directory read, no `self_key_id`, so it cannot fail open on unset node identity) wired at **both** placement doors on all three backends — `check_promotion_admission` (covering `promote_attestation` + `promote_attestation_transformed`, and therefore `Engine::attestation_promote`, `promote_consented_backlog`, the pyo3 wrapper and the FFI capsule) and `set_attestation_cohort_scope` (covering the #530 repair motion). The rule: **for `family` / `community`, the row must name no party but its own producer** — `attested_key_id == attesting_key_id` and every `subject_key_ids` entry is the producer. Typed refusal `Error::CohortStandingRefused` / `federation_cohort_standing_refused` carrying `CohortStandingRefusal::{attested_party, named_subject}`, so it names the FIELD that carried the foreign party rather than a membership record that was never the problem **v39.0.0 — the standing question moved to the WIDENING door, where AV-45 can finally ask it.** A crossing never places a row in a cohort plane (the placement is the row's own signed bytes), so the only way into `family`/`community` is now a `supersedes` written by `widen_audience` — which goes through `put_attestation`, where AV-45's write-scope gate proves MEMBERSHIP against the cohort the row names, and `check_promotion_cohort_standing` then refuses on STANDING. The schema half of the residual is closed for this path: `Audience::Family`/`Community` carry the cohort id, and a targeted audience without one is refused as not an audience. The B9 witness now proves membership first, so what it exercises is standing and nothing else. | **This is a different predicate from AV-45 and must stay one.** AV-45 asks "is the writer in the cohort it named?"; AV-84 asks "is this the promoter's own content?". Forcing one implementation onto both is the axis-fusion defect. **Deliberately NOT closed:** a producer that belongs to no community can still place its OWN content at `community`. That placement names no cohort — the row has no field to name one — so it is unprovable identically for everybody; closing it means naming the target end-to-end (grant grammar + promotion signature + a stored column if it is ever to be read back), and every route to it reddens `promotion_honors_audience_510` and `attestation_promote_rejects_federation_self_519`, which ARE instances of an unproven placement. **No migration was taken**: the target must be carried by the publish ACT in either design, so a stored column is the act-carrying change PLUS a column whose only consumer would be the read gate — a separate defect, and one with no honest backfill value for rows promoted before it existed. Note also that the read gate on this table is already documented as using `attested_key_id` as the membership target ("the subject doubles as the membership target"), so a family/community attestation is fail-closed at read rather than leaking | ✅ **Shipped** — B9 `{gate} × {backend}` matrix (a third-party row refused at `community` AND `family`, naming its branch; the SAME row still promotes at `federation`, so the targeted-cohort arm did not become a general third-party ban; a producer's own row still reaches the community plane, so the #510 audience plane is intact; both refusals byte-identical, including on memory where the gate had to run on a candidate clone; and the same verdict at the `set_attestation_cohort_scope` door), plus `promotion_cohort_standing_is_not_av45_592` (the separation, pinned) and `promotion_does_not_prove_cohort_membership_589` still green unmodified | CIRISPersist#592 / #589 / #519 / #510 |
| AV-85 | **A write path constructs signed bytes and then mutates the row.** CIRISPersist#643 bound seven typed columns into the signed envelope (`envelope.row`) and made `check_row_column_binding` refuse any row whose columns diverge from that mirror. `Engine::attestation_promote` and `promote_attestation_with_transforms` re-sign a row and CHANGE `cohort_scope` — one of the seven — and they were re-signing the PRE-promotion envelope, so every promoted row carried a signed mirror asserting its old scope beside a column saying otherwise. **The promoting node's own output was refused by every peer's `put_attestation`**, and promotion is the local→federation path, so this broke the thing promotion exists to do. `set_attestation_cohort_scope` (the #530 repair door) had the same shape, and its doc still claimed `cohort_scope` was "outside the signed envelope, so the scrub signature stays valid" — a sentence #643 had made false. `write_local_attestation` stamped no mirror at all, so a locally minted row was one this substrate's own (tier-blind) put door would refuse. **This is the THIRD instance of one shape**, after #598 (`assemble` sampling `Utc::now()` AFTER signing, for a column the envelope also carried) and #643's own blob-eviction sweeper (a hand-rolled emit recipe emitting rows its own gate refused) | v31.0.0 — SHIPPED (CIRISPersist#649): a placement-touching write is a RE-SIGN. `promote_attestation` and `set_attestation_cohort_scope` take an `AttestationReseal` — the re-stamped envelope, its digest, its signature, the scrub key and timestamp — and write the envelope in the SAME statement as the placement. `RowMirror::restamp_for_scope` builds it from `RowMirror::of`, the same projection the gate compares against; there is no second spelling. `promote_attestation_transformed` is GONE, absorbed into `promote_attestation`: it existed only because it also wrote the envelope back, and now every promotion must. `check_promotion_admission` gained `check_row_column_binding` — #643 could not put it there (the gate would have refused every promotion rather than fixing any), and making the rule satisfiable is what makes it enforceable, the #598 sequence exactly. The LOCAL door now stamps the mirror, because persist MINTS those bytes (it assigns `attestation_id` and defaults `attested_key_id`); the caller-signed TRANSIT revocation is excluded, because stamping bytes a caller already signed would be this defect one door over **v39.0.0 — this class is now unreachable at the crossing.** `enter_mesh` does not re-sign and does not touch `cohort_scope`: the bytes it stores are the bytes the actor signed, and offering re-stamped bytes as custody is refused by kind (`PromotionMovedThePreimage`, witness W1). The re-stamp survives only for `widen_audience`, which does not mutate a row at all — it writes a NEW `supersedes` through the ordinary emit recipe. | **The sweep found a FOURTH cluster and it is NOT closed here.** `Family` / `Community` sign their whole record minus `persist_row_hash`, so `members` / `family_name` / `founded_at` / `consensus_protocol` / `policy_blob` are all inside the signing preimage — and `add_family_member`, `add_community_member` and `supersede_group_row` mutate them on all three backends while leaving `authority_key_id` / `scrub_signature_*` untouched. `supersede_family` even RECEIVES a valid new `SignedFamily` and serializes only `.family`, dropping the caller's new signature. So a superseded or grown roster is served by `list_signed_families_since` under a signature covering a roster that no longer exists, and every peer's `put_family` → `verify_family_admission` refuses it — the #649 shape verbatim, one plane over, with the same local `Ok`. Verified by reading `Family::signing_envelope`, the two backends' `UPDATE` SET lists and `supersede_family`; NOT fixed in this cut and NOT witnessed. Adjacent and lesser: `attach_attestation_pqc_signature` mutates three hash-covered-but-unsigned columns without re-indexing, while its twin `attach_key_pqc_signature` does re-index (the #547/#640 stale-index class); and `blobs::emit_withdraws_attestation_helper` stamps the mirror from a hand-written `json!` literal rather than `RowMirror::of`, so the next member added to the closed mirror re-opens #643 there | ✅ **Shipped** — B11 `exercise_promoted_row_crosses_to_a_peer` on memory + sqlite + postgres, ending at a SECOND directory's `put_attestation`: local write → promote → **peer admits**. The negative arm feeds that same peer the pre-#649 row itself (promoted columns, envelope signed before the scope changed) and requires a refusal naming `cohort_scope`. Mutation-tested: removing the re-stamp turns it red at the primitive; removing the re-stamp AND the new gate turns it red at the PEER, quoting the peer's own refusal | CIRISPersist#649 / #643 / #598 |
| AV-86 | **The fabric signs an actor's claim.** `promote_attestation` re-signed a local-tier row with THIS NODE's key, overwrote `scrub_key_id`, and cleared every co-scrub. Under the 2-or-3-key model (NodeID at bootstrap; NodeID + FedID after startup; FedID + NodeID + AgentID for agents) that made the node the only signer of a human's or an agent's claim at the exact moment the claim became federation-visible: the actor's non-repudiable signature was destroyed by the operation whose purpose is to publish it, and what peers received was a node-attested assertion about what somebody else had said. It was also unreplicable whenever the attester was not the node — the wire verifier resolves the base signature against `attesting_key_id`, never `scrub_key_id` — so an agent's promoted row was refused at every peer while promotion returned `Ok`, which is what hid it. The deeper hazard is that a fabric able to re-sign at the crossing is a fabric able to publish, in the actor's name, a row the actor never signed | v39.0.0 — SHIPPED as a MAJOR. Two operations replace one: `enter_mesh` (CC 5.3.2.4.2) crosses `local → federation` over the SAME bytes with the ACTOR's signature as the base scrub, and the node may only APPEND a co-scrub (`additional_scrubs`, stamped `cosigned_at` OUTSIDE the preimage); `widen_audience` (CC 4.4.3.3.1) reaches a wider audience by a `supersedes` the actor signs, leaving the prior untouched. Custody is a closed enum decided in ONE place (`Engine::custody_for`): a node co-scrub over a row with no actor signature is refused (`NoActorSignature`), custody offered by a non-attester is refused (`CustodyIsNotTheActor`), and an unsigned row whose actor is absent WAITS (`MeshCrossingOutcome::AwaitingActor`) rather than being signed by the fabric. Every signature admitted — actor, node, and each inherited co-scrub — is verified at the crossing under `HybridPolicy::Strict`, so the preserve set equals the verified set (#541/#556) without clearing anything. `LocalAttestationInput::bind_for_signing` + `verify_caller_signed_local_row` let a software-held actor key sign at write, which is what makes the co-scrub path reachable at all | **What this does NOT close.** Nothing verifies that the key which signed a row is the key its HOLDER intended to be identified by: `custody_for` proves the signer IS the attester, not that the attester is who the human thinks they are — that is the ceremony's job (`reference_portable_trust_root_two_planes`), one plane up. And a node that holds an actor's software key can still sign as that actor; sign-at-write narrows the window (the signature exists before the crossing) but cohabitation means an agent's node CAN sign as its agent, by design (CC 4.4 two granters). What changed is that the fabric can no longer do it with its OWN key and have the result accepted | ✅ **Shipped** — W1–W14 (`FSD/PROMOTION_PRESERVES_THE_ACTOR_SIGNATURE.md` §6) at the directory primitive on all three backends via `exercise_actor_signature_survives_the_crossing`, each arm naming the mutation that must fail it; plus the #649 witness asserting a crossed row AND its widening admit at a PEER | CIRISPersist FSD/PROMOTION_PRESERVES_THE_ACTOR_SIGNATURE.md / CC 5.3.2.4.2 |
| AV-87 | **A resource control keyed on an axis the caller cannot be distinguished on.** The AV-76 per-peer write quota meters by `attesting_key_id`, at tier 0, ahead of any signature check — so the key is what the row CLAIMS. That is correct and deliberate as a flood shield, and it means the quota cannot tell three different parties apart: a hostile stranger, this node's own owner publishing locally, and an authenticated cohort peer handing over a backlog. v39.0.0 routed local publication through that door and the shield began metering the owner (652 of 900 chat sends refused, CIRISPersist#804); it also promoted the owner to a tracked peer bucket, so `tracked_peers > 0` lifted `node_state`'s peer-quota band into `green` on the strength of a node talking to itself — CIRISPersist#665's signal corruption, one identity over. The general hazard: **a control whose key is forgeable at the point it runs can only ever be as generous as it dares be to a liar**, so every legitimate party is metered as the worst one | v41.0.0 — SHIPPED. Origin is a property of the CALL, not the row, so it became a parameter: `put_attestation_with_origin` with `put_attestation` (wire, unchanged — AV-76 untouched), `put_attestation_authored` (node ceiling only; no peer bucket, no stranger's tail) and `put_attestation_synced` (a shared bulk budget for authenticated cohort peers) as named doors over it. The sync door's privilege is bounded by a fact persist checks for itself: the caller vouches only for WHICH identity its transport authenticated, and whether that identity is a cohort-mate is resolved from persist's own family/community rosters, memoized for one burst window. An authenticated stranger falls back to ordinary metering | **What this does NOT close.** The wire door is unchanged and still meters a claimed identity, because at tier 0 there is nothing better to key on — an unauthenticated flood is still charged to whoever it claims to be, and rotation still buys a stranger the shared tail rather than a fresh bucket (the `UNTRACKED_TAIL_BUDGET_MULTIPLE` argument). And the privileged doors are only as safe as the boundary that reaches them: they are in-process surfaces, never exposable across a wire | ✅ **Shipped** — budget sizes witnessed against a SIMULATED clock (`admission::local_authorship_804`), door routing witnessed on all three backends (`exercise_privileged_sync_door_804`), each with the mutation that must fail it. Two of the cut's own witnesses were rebuilt after mutation testing showed they measured refill and the node ceiling rather than the budgets they named | CIRISPersist#804 / #665 / AV-76 |
| AV-88 | **Unbounded `ContentFetch`: the puller reads to EOF and hashes after.** A puller doing a `ContentFetch` had no declared length — nothing on the wire said how many bytes a blob was — so it read to EOF and hashed afterwards, and a lying or hostile holder could stream unbounded bytes at it (memory / disk exhaustion) before the full-SHA check (CC 5.3.2.5) could refuse. Persist bounded the WRITE side (AV-7's 8 MiB body limit, AV-40's `body_size_bytes` CHECK, the inline-put cap, AV-76's cheapest-first tiering) and nothing bounded the PULL side; `size` was not a member of the `holds_bytes` claim or of any media descriptor, so there was no number a peer had already verified the signature over to cap by | v45.0.0 (CIRISPersist#871): `size` is a REQUIRED signed member of every `holds_bytes` claim and every media Source struct (CC 3.3.13: *"`size` (bytes) is REQUIRED — on every blob, not only media"*); the puller caps its read at the declared size and refuses on overrun/underrun BEFORE hashing (AV-76's cheapest-first discipline on the byte plane — CC 5.3.2.5: *"size is checked first"*); persist refuses a claim without a size at every door (`media_source::holds_bytes_claim_size` at ingest, `check_media_source` at the three ingest doors, the three local doors and the promotion chokepoint, kind `federation_media_source_invalid`, the member named) | **What this does NOT close.** Persist supplies the number in the signed bytes a peer already verifies; the cap itself is enforced by the puller (Edge's half of #871). A holder that declares the true size and then stalls is a liveness problem, not a size one (hop/TTL, #672). And `size` bounds ONE fetch: a holder that advertises many blobs is bounded by the per-peer quota (AV-76), not by this | ✅ **Shipped** — I115 (twenty malformed structs refused by name, incl. `size` absent / `0` / `"4096"`, on all three backends and at the local door), I117 (a `holds_bytes` claim without `size`, or with `0` / `"12"` / `-1`, refused at ingest on all three backends; the put and adopt doors' claims carry the exact stored length; a two-node delivery carries it to the peer), I119 (from disk: the gate at every door). The grammar's own unit tests were mutation-checked: dropping the `size == 0` refusal, the MUST-NOT scan, the codec-iff-MP4 rule, the captions citation and the claim-type guard each fails at least one test | CIRISPersist#871 / CIRISConstitution#104 / CC 3.3.13 / CC 5.3.2.5 / CC 5.3.2.6 |
| AV-89 | **Size divergence between the author's descriptor and a holder's claim.** The author's Source struct says one length and a holder's `holds_bytes` claim says another — the holder is lying, or holds a truncated copy under the same sha (which the digest would catch, but only AFTER the bytes were read). Before this cut `list_holders(sha)` returned bare key ids, so a puller had nothing to compare and picked a holder blind; the first thing it could learn about a holder's honesty was the digest mismatch at the end of the read | v45.0.0 (CIRISPersist#871): `list_holders_sized(sha) -> Vec<HolderClaim { key_id, size }>` returns each holder's claimed size beside the author's; a puller refuses a holder whose claim disagrees with the descriptor it is fetching for, before any bytes move; persist's own adopt door refuses to announce a claim whose `size` is not the byte length it just stored (`put_blob_with_scope` / `adopt_sealed_blob_at` pass the stored length into the ONE `holds_bytes_attestation_row` definition and the signer signs it). `list_holders(sha)` is unchanged (Edge compiles) | **What this does NOT close.** Two holders that AGREE on a wrong size (colluding, or copies of one truncation) are refused only by the digest, as before — size is *"not a substitute for the digest, only the cheaper check that runs first"* (CC 5.3.2.5). And persist compares at its OWN adopt door only; a holder elsewhere that signs a wrong size is refused by the puller, not by persist | ✅ **Shipped** — I117d (the refusal itself, on sqlite and postgres: a claim whose `size` is not the byte length the door stores is refused at the put door and at the adopt door before anything is written; the signer refuses a zero size; the true length is admitted), I117b (the put door's and the adopt door's claims carry the exact stored byte length; `list_holders_sized` returns it; the claim crosses to a peer with the size via the #870 hashes → point reads → apply shape), I116 (`evidence_refs` entries stay bare sha256 strings — an object `{sha, size}` entry is not a citation, pinned so the ask-2 shape cannot creep in) | CIRISPersist#871 / CC 3.3.13 / CC 5.3.2.5 / AV-88 |
...
v45.0.0 SIZE ON EVERY BLOB CLAIM (CIRISPersist#871)
  ✓ AV-88 unbounded ContentFetch (size is a REQUIRED signed member of every holds_bytes
          claim and media Source struct; the puller caps its read BEFORE hashing;
          persist refuses a claim without a size at every door)
  ✓ AV-89 author/holder size divergence (list_holders_sized returns the holder's claimed
          size beside the descriptor's; the puller refuses a disagreeing holder;
          the adopt door never announces a size ≠ the length it stored)
