...
- **`BlobStorage::put_scope_blob` + a read path + LRU eviction** (FSD §2.4 symbol-AEAD store). The substrate half of [CEWP `FSD/SCOPE_PRIVACY.md`](https://github.com/CIRISAI/CEWP/blob/main/FSD/SCOPE_PRIVACY.md) — the CIRIS Constitution **CC 1.13.3** anonymous-tier opt-in mechanism. New table `federation_scope_blobs` (migration **V088**, both dialects) holds caller-pre-encrypted (XChaCha20-Poly1305) RaptorQ symbols at community/family/self scope, addressed by `(record_id, symbol_index)` (the PK; `record_id` = FSD §2.4 HMAC-SHA3-256 output, `symbol_index` = `0..N`, N=20 default).
  - **persist stores opaque ciphertext only.** The seal (`nonce` / `ciphertext` / `tag`) and the `record_id` / `symbol_key` derivation are entirely **CIRISEdge-side** (the producer side is the CIRISVerify **v6.3.0** scope-privacy crypto — HKDF-SHA3 / HMAC-SHA3 / XChaCha20-Poly1305 / X-Wing HPKE; persist consumes none of it). **This slice has ZERO verify-crypto dependency — the verify pin stays v6.2.0.**
...
- **Deferred (out of this cut):** part 3 `EncryptedKVStore` / SQLCipher openmls `StorageProvider` (needs CIRISVerify **v6.3.0** + a hardware-key custodian); part 4 cohort-lattice contraction (BLOCKED on CIRISRegistry **CEG §11** ratification); part 5 the verify v6.3.0 re-pin (v6.3.0 not tagged yet).
