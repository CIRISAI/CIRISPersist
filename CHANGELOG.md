...
- **Out-of-order federation delivery** (design choice, flagged): when target `T`
  ...
  semantics. The 4-rule gate is fully enforced whenever `T` IS local.
...
- postgres (1, green against lead's docker PG): rule-2 + §8.1.11.2 multi-subject
...
- Full feature set (`postgres sqlite server pyo3 cirisaudit secrets cirisnode
  ...
  `takedown_handler` tests that the unscoped gate had regressed.
