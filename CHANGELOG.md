...
### Added — Cut B (enumerators + add_community_member)

- **CEG-native graph DX — the missing read-side enumerators + the incremental community-roster grow (CIRISPersist#249 Cut B).** Each new reader is the enumeration companion of an existing predicate / forward-only walk, so the substrate can answer "give me the set" not just "is X in the set":
  - **`FederationDirectory::active_community_members(community_key_id)` + `active_family_members(family_key_id)`** — a group's roster MINUS its effective (`effective_at <= now`) membership revocations (a future-dated revocation does NOT drop its subject). Default trait methods composing `lookup_*` + `list_*_membership_revocations_for`. The roster-minus-effective-revocations subtraction is **factored** into one shared free function `crate::federation::removed_key_ids_at`, which the community-DEK cascade's `resolve_community_members` (`src/federation/community_dek.rs`) now composes too — the forward-secrecy subtraction is **not forked**. Distinct from `list_*_for_member_active` (the inverse: groups-of-a-member).
  - **`admission::moderators_of(community_id, duty)`** — the FULL named-moderator set: owner-bound authority roots ∪ every delegate they reach via a live `duty`-scoped `delegates_to` chain. The enumeration companion of the `is_named_moderator` predicate (`k ∈ moderators_of(…)` ⟺ `is_named_moderator(k, …)`), built from a new `enumerate_scoped_delegation_reach` BFS that shares the exact `MODERATION_DUTY` walk policy (attenuation + `sub_delegation`-gated deputization + withdrawn-edge skipping + depth cap) the predicate uses.
  - **`admission::reachable_under_scope(issuer, target, scope, max_depth)`** — the **general scoped-delegation reachability primitive** (the by-construction "#13" load-bearing walk every delegated-duty reader specializes: `moderators_of` / `is_named_moderator` root it at a community authority set, `owner_bindings_of` reads the human anchor, `duty_holders_*` compose it). Makes persist's previously-PRIVATE `issuer_reaches_target_via_scoped_delegation` walk public — a **thin `pub` wrapper** (singleton target set + the `MODERATION_DUTY` policy), with **zero change** to the ⊆-attenuation / `sub_delegation`-gated deputization / `withdraws`-aware / depth-cap (`min(max_depth, MAX_WITHDRAWS_DELEGATION_DEPTH)`) semantics. Scope-isolation holds (a `moderate` chain is not reachable under `review`); zero-hop `issuer == target` is not a reach.
  - **`admission::owner_bindings_of(key_id)`** — the `user`-role key(s) that owner-bind `key_id` (who it is owner-bound TO), enumerating the same three clauses `is_owner_bound` tests (`!owner_bindings_of(k).is_empty()` ⟺ `is_owner_bound(k)`).
  - **`admission::owner_binding_chain(key_id)`** — the owner-binding **PATH** for audit (`user → … → key_id`, anchor-first), the chain not just the endpoints: clause 1 (key is user-role) → `[key]`; clause 2 (occurrence of a user identity) → `[identity, key]`; clause 3 (live `delegates_to(user → key)`) → `[user, key]`. Resolves the first satisfying clause in `is_owner_bound`'s precedence (`!owner_binding_chain(k).is_empty()` ⟺ `is_owner_bound(k)`).
  - **`FederationDirectory::delegations_to(key_id)`** — the inbound `delegates_to` edges naming `key_id` as recipient ("who delegated TO me?"), the reverse of the forward-only `topology::build_delegation_graph`. Default trait method filtering `list_attestations_for`.
  - **`FederationDirectory::add_community_member(community_key_id, member)`** — incremental community-roster grow, the exact mirror of `add_family_member`: append to the roster, idempotent on `member.key_id` (`Ok(false)` no-op / `Ok(true)` genuine add), recompute `persist_row_hash`. New pg / sqlite / memory impls (today only whole-re-put `put_community` existed).
  - **3-backend parity** (pg JSONB + sqlite TEXT + memory Vec) and **FFI** for each (`active_community_members_json` / `active_family_members_json` / `moderators_of_json` / `owner_bindings_of_json` / `owner_binding_chain_json` / `reachable_under_scope` / `delegations_to_json` / `add_community_member`), plus thin `Engine` convenience wrappers. Tests on all three backends: roster N → revoke-1 → N−1, future-dated revocation keeps the member, moderator roots ∪ delegates with the outsider excluded, owner-bound vs unbound + the audit chain path, `reachable_under_scope` reaching a 2-hop `moderate` chain with scope-isolation + depth-cap behavior, 2-inbound-delegate enumeration, and add → appears / re-add → idempotent.
