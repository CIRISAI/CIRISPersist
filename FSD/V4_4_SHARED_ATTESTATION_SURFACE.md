...
**Status:** **Phase 1 ACCEPTED + Phase 2 UNBLOCKED** (2026-06-08) — all four CEG-RC1 implementations nod (Agent ✅, Registry ✅, LensCore ✅, NodeCore ✅; Verify ✅ on OQ-4). The write+read half (tier model + `upsert_local`/`insert_local`/`query` + write gates + §5 scan/emission + migration) is cleared to build. **Phase 2 (`attestation_promote`)** is now also cleared: **CIRISVerify#59 closed → v4.11.0 ships `ciris_verify_core::jcs::{canonicalize, verify_jcs_hybrid_signature}`** (RFC 8785 + CEG-Contribution hybrid-sig verify), the OQ-4 deliverable. Persist re-pins 4.10.0 → 4.11.0 and `promote` canonicalizes via `jcs::canonicalize` (§4.3). **Phase-1 clock trigger (§5)** still pending OQ-3 / CIRISPersist#161 (trigger-only). Surface pinned in CEG §10.1.5 (`v-ceg-0.15`).
...
**Dependency — RESOLVED (CIRISVerify#59 closed, v4.11.0).** Persist re-pins CIRISVerify 4.10.0 → **4.11.0** and computes the promote-time bytes via **`ciris_verify_core::jcs::canonicalize(&envelope)`** (RFC 8785, the shared cross-impl impl — so persist's producer bytes are byte-identical to what a peer recomputes); the federation-tier signature is `Engine::sign_hybrid(jcs::canonicalize(envelope))`, and verifying peers use `ciris_verify_core::jcs::verify_jcs_hybrid_signature`. This is routed through the `secrets::crypto` / verify facades per MISSION §1.4 (no rolled crypto, no hand-rolled JCS). `promote` may now build alongside the write+read half — though it MAY still ship second behind phase 1 for review-staging convenience (OQ-5), it is no longer *blocked*. Composes with CIRISConformance#9's RFC 8785 vector set.
...
- **OQ-4 — promotion canonical bytes. ✅ RESOLVED + dependency shipped.** `JCS(envelope)` per CEG §0.9 / RFC 8785 (3 Registry musts, §4.3) — NOT Verify's internal LP framing. **CIRISVerify#59 closed → v4.11.0 ships `ciris_verify_core::jcs::{canonicalize, verify_jcs_hybrid_signature}`.** Persist re-pins 4.11.0 + canonicalizes via `jcs::canonicalize`. `promote` is no longer blocked (may build with phase 1 or stage second for convenience). Registry owns the envelope member set; CEG §10.1.5.3 pins it.
...
| **CIRISVerify** | **✅ DELIVERED** | Promotion = `JCS(envelope)` §0.9, not LP framing. **CIRISVerify#59 closed → v4.11.0 ships `jcs::{canonicalize, verify_jcs_hybrid_signature}`.** Persist re-pins 4.11.0. Phase-2 dependency satisfied. OQ-4 ✅. |
...
**Phase 1 (write+read) is accepted — all four impls nod, zero remaining blockers.** Build it: tier model + `upsert_local`/`insert_local`(+`_many`) + `query` + §4.1 gates + §5 scan/emission + migration. **Phase 2 (`promote`) is now unblocked too** — CIRISVerify#59 closed (v4.11.0 JCS); it may build with phase 1 or stage second for review convenience. **OQ-3 (#161)** still picks the §5 clock trigger (persist ships scan+emission regardless).
