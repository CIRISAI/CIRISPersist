...
**Status:** **SHIPPED in v39.0.0** (2026-09-02); corrected by **v40.0.0** (CIRISPersist#801) and **v41.0.0** (CIRISPersist#804) — see §11 items 11 and 12. The build followed this document with the deltas recorded in §11; read §11 before citing any earlier section as the shipped shape. Edge reviewed end-to-end and answered OQ-1/OQ-2 with corrections folded below (CIRISEdge `docs/FSD_REPLICATION_DX.md` §6 items 4–6, `786815c`). Written against CIRISConstitution **1.0-rc4** and persist **v38.8.0**.
...
11. **A widening carries the claim's `asserted_at`; the placement's own instant is a new signed member `widened_at`** (v40.0.0, CIRISPersist#801, reported by CIRISServer). v39.0.0's `build_widening` treated `asserted_at` as placement bookkeeping and dropped it, so `stamp_and_canonicalize` minted a fresh one. Because a local row must be `self`-scoped and `self` never replicates, the widening is the only row a peer sees — so the claim's instant became **unrecoverable off-node**, and every cross-node reader comparing "claim times" was in fact comparing placement times. `differs_in: ["cohort_scope"]` was false the whole time, and the gate could not see it because `is_placement_member` exempted `asserted_at` from the very comparison that would have caught it. Fixed by carrying the prior's instant verbatim, adding `widened_at` to the agreed vocabulary (re-pinning `ENVELOPE_VOCABULARY_SHA256`, hence MAJOR), and having `assemble` read `scrub_timestamp` from `widened_at` — which is why the act's instant had to be a *signed member* rather than a column: preserving `asserted_at` alone would have erased the placement time too. Witnesses W15/W16 plus a peer-side assertion; both mutations verified.

12. **Routing local publication through the ingest door inherited its peer metering** (v41.0.0, CIRISPersist#804, reported by CIRISServer). This FSD moved local publication onto `put_attestation` — `widen_audience` writes through it — and `put_attestation` is where the AV-76 per-peer write quota lives. The quota is keyed on `attesting_key_id` with no origin axis, so from v39.0.0 the node's own owner was metered by the control that exists to stop a hostile remote peer flooding this node: **652 of 900 chat sends refused**, and `tracked_peers` corrupted into reporting a tested quota on the strength of the owner talking to itself.

    The axis is ORIGIN, and it is not recoverable from the row — the quota runs at tier 0, before any signature check, so `attesting_key_id` is a claim (the trap `GenesisClaim` already documents). So origin became a parameter: `put_attestation_with_origin` is the one required method, with `put_attestation` (wire, unchanged) and `put_attestation_authored` (this node signed it — node ceiling only, no peer bucket, no stranger's tail) as defaults over it. None of the 662 existing call sites changed.

    A third door followed from the same reasoning: `put_attestation_synced(row, authenticated_peer)`, for bulk cohort sync, which is legitimately bursty and which the authored door cannot serve because a sync carries rows authored by many parties. Its privilege is bounded by a fact persist checks itself — the caller vouches only for which identity its transport authenticated; whether that identity is a cohort-mate comes from persist's own rosters, and an authenticated stranger falls back to ordinary metering.

    **The lesson for this FSD specifically:** a crossing verb that publishes through the ingest door inherits every control written for ingest. §5 reasoned carefully about which admission gates a crossing should re-run and said nothing about the quota, because the quota is not an admission gate — it is a resource control, and it was keyed on an axis the crossing silently changed the meaning of.
