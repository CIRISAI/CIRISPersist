...
## 12. Bundled from the backlog pass: CIRISVerify v17.1, #913, #911, #915

**CIRISVerify v16.2.1 → v17.1.0.** Seven Cargo pins and the wheel's `ciris-verify>=17.1.0,<18` move together. (v17.1.0, taken mid-release, fixes a lagging replica being reported as a possible attack, CIRISVerify#223; persist does not call it.) v17 makes `ciris_crypto::Ed25519Verifier`'s `ClassicalVerifier::verify` strict (CIRISVerify#292's supersede fix and #293's Android challenge policy ride the same tag).
